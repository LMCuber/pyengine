...
def choose_file(title, file_types="all"):
    ...
    if Platform.os != "Darwin":
        pth = tkinter.filedialog.askopenfilename(title=title, **kw)
        return pth
    else:
        ft = {ft[1].removeprefix("*.") for ft in file_types}
        proc = subprocess.Popen(["osascript", "-e", f'log choose file of type "{ft}"'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        comm = proc.communicate()
        pth = path(*comm[1].decode().removesuffix("\n").split(":"))
